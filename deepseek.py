...
class Gate:
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.n_groups > 1:
            ...
            scores = scores.masked_fill_(mask.unsquueze(-1), float("-inf")).flatten(1)
...
class MLP(nn.module):
    """
    Multi-Layer Perceptron(MLP) as a feed-forward layer
...
class MoE(nn.Module):
    ...
    Attributes:
        ...
        gate (nn.Module): Gating mechanism to route input to experts.
    ...
    def __init__(self, args: ModelArgs):
        super().__init__()
        self.dim = args.dim
        assert args.n_routed_experts % world_size == 0, f"Number of experts must be divided by world_size (world_size={worls_size})"
        ...
        self.gate = Gate(args)
        self.experts = nn.ModuleList([Expert(args.dim, args.moe_inter_dim) if self.expert_start_idx <= i < self.experts_end_idx else None
    ...
    def forward(self, x: torch.Tensor)-> torch.Tensor:
        ...
        counts = torch.bicount(indices.flatten(), minlength=self.n_routed_experts).tolist()
